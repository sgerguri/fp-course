...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  g <$> State { runState = f } = State { runState = \s -> let (a,s') = f s in (g a, s') }
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  State { runState = f } <*> State { runState = g } = State { runState = \s -> let (h, s') = f s
                                                                                   (a, s'') = g s' in (h a, s'') }
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  f =<< State { runState = g } = State { runState = \s -> let (a, s') = g s in runState (f a) s' }
...
findM ::
  ...
  -> List a
  -> f (Optional a)
findM p xs =  listToOptional <$> filtering p xs
  where listToOptional Nil = Empty
        listToOptional (x :. _) = Full x
