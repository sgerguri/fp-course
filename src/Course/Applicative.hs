...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) fs xs = flatten $ map mapXs fs
                where mapXs f = map f xs
...
instance Applicative ((->) t) where
  ...
  (<*>) ::
    ...
    -> ((->) t a)
    -> ((->) t b)
  (<*>) constff constf = \x -> constff x $ constf x
...
lift2 ::
  ...
  -> f b
  -> f c
lift2 fab fa fb = pure fab <*> fa <*> fb
...
(*>) ::
  ...
  -> f b
(*>) fa fb = (\_ y -> y) <$> fa <*> fb
--This works as well, since <$> = <*> . pure:
--(*>) fa fb = pure (\_ y -> y) <*> fa <*> fb
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering pf = foldRight (\a -> lift2 (\x -> if x then (a:.) else (\y -> y)) (pf a)) (pure Nil)
