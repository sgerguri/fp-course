...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = seqOptional' (Full Nil)
    where seqOptional' (Full ys) (Full x :. xs) = seqOptional' (Full (x :. ys)) xs
          seqOptional' (Full ys) Nil = Full (reverse ys)
...
--  Hypothesis: Impossible. Proof through second prop and RHS of first prop, implying LHS
--  of first prop
